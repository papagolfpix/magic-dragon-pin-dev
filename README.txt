MAGIC DRAGON PIN v0.10.11 DEV — SUPABASE STAGE 1 URL FIX

Purpose:
Development/staging build only. Production remains separate.

What changed in v0.10.11 DEV:
...
Hotfix v0.10.11: Service worker now bypasses all cross-origin API traffic so Supabase authentication is handled directly by Safari instead of the offline asset cache.


v0.10.11 DEV diagnostic: service worker intentionally disabled/unregistered to eliminate stale Safari worker interference while validating Supabase authentication.


Fix v0.10.11: corrected Supabase DEV hostname from bzqkeshxbnhnlrpdgbtb.supabase.co to bzgkeshxbnhnlrpdgbtb.supabase.co. The prior one-character hostname typo caused Safari fetch() to fail before authentication with “Load failed”.
